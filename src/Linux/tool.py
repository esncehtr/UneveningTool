    ...
    try:
        for slot in ['a', 'b']:
            for file, part in zip(files, ['vbmeta', 'vbmeta_system', 'vbmeta_vendor']):
                step += 1
                progress(step, total)
                subprocess.run(["fastboot", "--disable-verity", "--disable-verification", "flash", f"{part}_{slot}", f"{folder}/{file}"], check=True)
